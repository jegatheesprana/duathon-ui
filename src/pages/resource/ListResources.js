	...
	return (
		<>
			...
			<DataGridTable
				...
				customActions={[
					{
						label: ({ status }) => status ? "Disable Resource" : "Enable Resource",
						icon: ({ status }) => status ? <CloseIcon /> : <CheckIcon />,
						conformation: {
							title: ({ status }) => status ? "Disable Resource" : "Enable Resource",
							message: ({ status, title }) => `Confirm ${status ? "Disable" : "Enable"} the ${title} resource?`
						},
						url: ({ _id }) => `/resources/status/${_id}`,
